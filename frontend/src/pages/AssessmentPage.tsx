...
interface FormData {
  ...
  dietary_pattern: string; meals_per_day: number; water_intake_liters: number
  daily_fruit_vegetable_servings: number; junk_food_frequency: string
  activity_level: string; sleep_hours_per_night: number; smoking_status: string
  alcohol_consumption: string; sunlight_exposure_min_per_day: number; stress_level: number
...
export default function AssessmentPage() {
  ...
  const [form, setForm] = useState<FormData>({
    age: 30, gender: 'MALE', height_cm: 170, weight_kg: 70,
    dietary_pattern: 'OMNIVORE', meals_per_day: 3, water_intake_liters: 2.0,
    daily_fruit_vegetable_servings: 3, junk_food_frequency: 'RARELY',
    activity_level: 'MODERATELY_ACTIVE', sleep_hours_per_night: 7,
    smoking_status: 'NEVER', alcohol_consumption: 'NONE',
    sunlight_exposure_min_per_day: 30, stress_level: 5,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        age: Number(form.age), gender: form.gender, height_cm: Number(form.height_cm), weight_kg: Number(form.weight_kg),
        dietary_habits: {
          dietary_pattern: form.dietary_pattern, meals_per_day: Number(form.meals_per_day),
          ...
          junk_food_frequency: form.junk_food_frequency, dietary_restrictions: [],
        },
        lifestyle_factors: {
          activity_level: form.activity_level, sleep_hours_per_night: Number(form.sleep_hours_per_night),
          smoking_status: form.smoking_status, alcohol_consumption: form.alcohol_consumption,
          sunlight_exposure_min_per_day: Number(form.sunlight_exposure_min_per_day), stress_level: Number(form.stress_level),
        },
      ...
      setSubmitError(err?.response?.data?.detail || err?.message || 'Assessment submission failed. Please verify connection and try again.')
  ...
  const steps = [
    <div key="personal">
      <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.375rem', fontWeight: 700, marginBottom: 8, letterSpacing: '-0.02em' }}>Personal Information</h2>
      <p style={{ fontSize: '0.875rem', color: 'var(--c-muted)', marginBottom: 28 }}>Basic demographics for baseline physiological ranges.</p>
    ...
    <div key="dietary">
      ...
      <p style={{ fontSize: '0.875rem', color: 'var(--c-muted)', marginBottom: 28 }}>Your dietary patterns affect nutrient intake and absorption.</p>
    ...
    <div key="symptoms">
      ...
      <p style={{ fontSize: '0.875rem', color: 'var(--c-muted)', marginBottom: 28 }}>Select any symptoms you're currently experiencing and rate their severity.</p>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 8 }}>Health Assessment</h1>
      <p style={{ fontSize: '0.875rem', color: 'var(--c-muted)', marginBottom: 32 }}>Complete the screening questionnaire to receive AI-powered nutrient deficiency predictions.</p>
      {submitError && (
        <div style={{
      ...
      <div className="card" style={{ padding: 32, marginBottom: 24 }}>
        <AnimatePresence mode="wait">
          <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit">{steps[step]}</motion.div>
        </AnimatePresence>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button onClick={prev} disabled={step === 0} className="btn-ghost" style={{ padding: '10px 24px', fontSize: '0.8125rem', opacity: step === 0 ? 0.4 : 1 }}><ChevronLeft size={14} /> Previous</button>
        {step < 5 ? (
          <button onClick={next} className="btn-primary" style={{ padding: '10px 24px', fontSize: '0.8125rem' }}>Next <ChevronRight size={14} /></button>
        ) : (
          <button onClick={handleSubmit} disabled={submitting} className="btn-primary" style={{ padding: '10px 28px', fontSize: '0.8125rem', opacity: submitting ? 0.7 : 1 }}>{submitting ? 'Analyzing...' : 'Submit Assessment'} <Send size={14} /></button>
        )}
      </div>
    </div>
  )
